...
test("internal Mission Control route renders Chinese owner dashboard with auto sync data", async ({
  ...
  await expect(page.getByText("Dispatcher Summary", { exact: true })).toBeVisible();
  await expect(page.getByText("Hermes Dispatcher", { exact: true })).toBeVisible();
  ...
  await expect(dispatcherQueueSummary.getByText("Task Count", { exact: true })).toBeVisible();
  await expect(dispatcherQueueSummary.getByText("Pending", { exact: true })).toBeVisible();
  await expect(dispatcherQueueSummary.getByText("Skipped", { exact: true })).toBeVisible();
  await expect(dispatcherQueueSummary.getByText("Safety Summary", { exact: true })).toBeVisible();
  await expect(dispatcherQueueSummary.getByText("merge disabled: true", { exact: true })).toBeVisible();
  await expect(dispatcherQueueSummary.getByText("deploy disabled: true", { exact: true })).toBeVisible();
  await expect(dispatcherQueueSummary.getByText("Task Preview", { exact: true })).toBeVisible();
  const runnerSummary = page.getByLabel("Runner Summary");
  await expect(runnerSummary.getByText("Hermes Runner", { exact: true })).toBeVisible();
  await expect(runnerSummary.getByText("Task Count", { exact: true })).toBeVisible();
  await expect(runnerSummary.getByText("Completed", { exact: true })).toBeVisible();
  await expect(runnerSummary.getByText("Failed", { exact: true })).toBeVisible();
  await expect(runnerSummary.getByText("Skipped", { exact: true })).toBeVisible();
  await expect(runnerSummary.getByText("Safety Mode", { exact: true })).toBeVisible();
  await expect(runnerSummary.getByText("push disabled: true").first()).toBeVisible();
  await expect(runnerSummary.getByText("destructive disabled: true").first()).toBeVisible();
